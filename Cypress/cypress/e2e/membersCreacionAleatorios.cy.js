...
describe('Testing funcionalidad members', () => {
  ...
  it('Probar la creación de un member con faker 1', () => {
    ...
    cy.get('input[name="name"]').should('exist').should('be.visible').type(faker.internet.email());
    cy.wait(2000);
    cy.screenshot('memberE3')
    cy.get('input[name="email"]').clear();
    cy.get('input[name="email"]').should('exist').should('be.visible').type(faker.name.firstName());
    cy.wait(2000);
